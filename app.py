...
if "scanned_id" not in st.session_state: st.session_state["scanned_id"] = 1
                    ...
                    if st.button("Onayla ve Yükle", use_container_width=True):
                        with st.spinner("Aktarılıyor..."):
                            ...
                            for r_i in range(header_row, len(df_raw)):
                                ...
                                if ad.lower() in yasakli_kelimeler and yazar.lower() in yasakli_kelimeler: continue
                            ...
                            conn_imp.commit()
                            conn_imp.close()
                            st.success(f"🎉 {len(ekler)} kitap aktarıldı, {atlanan} mükerrer atlanan var.")
                            st.rerun()
...
with tab_emanet:
    ...
    islem_tipi = st.radio("Yapmak İstediğiniz İşlem:", ["Emanet Ver", "Emanetten Geri Al"], horizontal=True)
    st.markdown("---")
    ...
    with st.expander("veya Manuel / QR ile ID Girin"):
        kitap_id_manual = st.number_input("Kitap ID:", min_value=1, step=1, value=int(st.session_state["scanned_id"]), key="input_manual_id")
        if st.button("Bu ID'yi Kullan"): secilen_kitap_id = kitap_id_manual
        ...
        if kamera_foto is not None:
            ...
            if decoded_info:
                ...
                if match:
                    ...
                    st.success(f"🎯 QR Kod Okundu! Kitap ID: #{found_id}")
