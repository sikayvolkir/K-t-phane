...
with tab_emanet:
    ...
    # URL parametrelerini kontrol et (HTML5 QR okuyucu url üzerinden id döndürür)
    ...
    if st.button("İşlemi Onayla ve Kaydet", use_container_width=True, key=f"btn_onayla_{ek_key}"):
        ...
        if secilen_id is None:
            st.warning("Lütfen işlem yapılacak bir kitap seçin veya QR okutun.")
        else:
            c.execute("SELECT id, ad,
