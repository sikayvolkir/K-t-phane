...
with tab_emanet:
    ...
    # URL parametrelerini kontrol et (HTML5 QR okuyucu url üzerinden id döndürebilir)
    ...
    if "qr_id" in query_params:
        try:
            ...
            st.toast(f"🎯 QR Kod Yakalandı! Kitap ID: #{qr_scanned_id}", icon="✅")
            st.rerun()
        except ValueError:
            pass
    ...
    # --- CANLI VİDEO QR KAMERA TARAYICI (HTML5 JS) ---
    if not st.session_state["kamera_acik"]:
        ...
        st.caption("📷 Kameraya QR Kodu Gösterin (Fotoğraf çekmeye gerek yoktur, otomatik taranır):")
        
        # HTML5 QR Kod Canlı Tarayıcı Bileşeni
        html_qr_scanner = """
        <script src="https://unpkg.com/html5-qrcode"></script>
        <div id="reader" style="width: 100%; max-width: 450px; margin: auto; border-radius: 10px; overflow: hidden;"></div>
        <script>
            function onScanSuccess(decodedText, decodedResult) {
                // QR Kod içinde KITAP_ID:X veya doğrudan sayı taranırsa
    ...
    # --- İŞLEMİ KAYDET BUTONU ---
    if st.button("İşlemi Onayla ve Kaydet", use_container_width=True, key=f"btn_onayla_{ek_key}"):
        secilen_id = st.session_state.get("selected_kitap_id")
        
        if secilen_id is None:
            st.warning("Lütfen işlem yapılacak bir kita
