...
with tab_emanet:
    ...
    if uygun_kitaplar:
        ...
        if islem_tipi == "Emanet Ver":
            ...
            st.info("Şu an emanette kitap yok.")
    ...
    with st.expander("veya Manuel / QR ile ID Girin"):
        ...
        if st.button("Bu ID'yi Kullan", key=f"btn_id_kullan_{ek_key}"):
        ...
        if kamera_foto is not None:
            ...
            if decoded_info:
                ...
                if match:
                    ...
                    st.warning("Geçerli bir Kitap ID bulunamadı.")
            else:
                st.error("⚠️ QR Kod tespit edilemedi.")
